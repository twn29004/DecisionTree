...
from process_data import read_iris_data
...
def split_discrete_dataset(dataset, idx, value):
    ...
    args:
        ...
        value: 划分属性的值
    return:
        true_dataset: 选择出符合条件的数据集和不符合的数据集,结构与原始数据集一致
    '''
...
def using_entropy_choose_best_attribute(dataset):
    ...
    for idx in range(length_attribute):
        ...
        if type(dataset[0][idx]).__name__ == "float" or type(dataset[0][idx]).__name__ == "int":
        ...
        # 对于离散值的划分
            ...
            tmp_info_gain = base_entropy
            for value in setValue:
                true_dataset = split_discrete_dataset(dataset, idx, value)
                tmp_info_gain -= float(len(true_dataset)) / len(dataset) * cal_entropy(true_dataset)
            if tmp_info_gain >= best_info_gain:
                best_info_gain = tmp_info_gain
                best_attribute_idx = idx
                best_attribute_value = None
...
def create_tree(dataset, attributes, criterion="gini"):
    ...
    # 如果是离散值的话
        ...
        setValue = {object[best_attribute_idx] for object in dataset}
        for value in setValue:
            true_dataset = split_discrete_dataset(dataset, best_attribute_idx, value)
            true_attributes = copy.deepcopy(attributes)
            if 0 == len(true_dataset):
                decision_tree[best_attribute][value] = majorCnt(label_list)
            else:
                decision_tree[best_attribute][value] = create_tree(true_dataset, true_attributes, criterion)
...
def classify_single(decision_tree, attributes, object):
    ...
    value = list(subTree.keys())[0]
    # 判断value的类型，连续值还是离散值
    if type(value).__name__ == "tuple":
        # 连续值
        if object[attribute_idx] < value[0]:
            flag = "less"
        else:
            flag = "more"
        subTree_key = tuple([value[0], flag])
    else:
        subTree_key = object[attribute_idx]
        # 如果当前的值不属于任何一个分支，则随机选择一个分支继续
        if subTree_key not in subTree.keys():
            length = len(list(subTree.keys()))
            idx = random.randint(0, length - 1)
            subTree_key = list(subTree.keys())[idx]

    
    # 子树的key值一定在子树的key值列表中
    assert subTree_key in subTree.keys(), "File: tree.py \nFunction: classify_single()\nsubTree key error{}\n{}"\
        .format(subTree_key, subTree.keys())
